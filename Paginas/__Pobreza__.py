...
def plot_pobreza_indigencia(data):
    ...
    fig.add_trace(go.Scatter(x=data.index[:10],y=data.loc[:'II Sem. 2007']["Pobreza"]*100,name="Pobreza",line=dict(width=2.5),marker_color="indigo",legendgroup="Pobreza",fillcolor="#BEADFA",fill="tozeroy",mode="lines"))
    fig.add_trace(go.Scatter(x=data.index[:10],y=data.loc[:'II Sem. 2007']["Indigencia"]*100,name="Indigencia",line=dict(width=2.5),fill="tozeroy",legendgroup="Indigencia",marker_color="#665A48",fillcolor="#D0B8A8",mode="lines"))
