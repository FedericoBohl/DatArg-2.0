...
def plot_ingresos(data:pd.DataFrame):
    ...
    fig.add_trace(go.Scatter(x=data.index,y=data["Linea Indigencia"].apply(lambda x: 0 if x==None else x),name="Línea de indigencia",fill="tozeroy",fillcolor="#F38BA0",line=dict(width=2),mode="none"))
    fig.add_trace(go.Scatter(x=data.index,y=data["Linea Pobreza"].apply(lambda x: 0 if x==None else x),name="Línea de Pobreza",fill="tozeroy",line=dict(width=2),mode="none"))
    fig.add_trace(go.Scatter(x=data.index,y=data["Canasta Basica"].apply(lambda x: 0 if x==None else x),name="Canasta Básica",fill="tozeroy",fillcolor="#B2B8A3",line=dict(width=2),mode="none"))
