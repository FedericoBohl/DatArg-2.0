...
def plot_emae(data:pd.DataFrame,var_m:pd.DataFrame,var_a:pd.DataFrame):
    ...
    fig_emae.add_trace(go.Bar(x=data.index,y=var_m['EMAE'],name='Var. Mensual',marker_color=var_m['EMAE'].apply(lambda x: 'green' if x >= 0 else 'red'),opacity=0.5,marker_line_color=black,marker_line_width=1),secondary_y=True)
    ...
    fig_campo.add_trace(go.Bar(x=data.index,y=var_m['Campo'],name='Var. Mensual',marker_color=var_m['Campo'].apply(lambda x: 'green' if x >= 0 else 'red'),opacity=0.5,marker_line_color=black,marker_line_width=1),secondary_y=True)
    ...
    fig_minas.add_trace(go.Bar(x=data.index,y=var_m['Minas'],name='Var. Mensual',marker_color=var_m['Minas'].apply(lambda x: 'green' if x >= 0 else 'red'),opacity=0.5,marker_line_color=black,marker_line_width=1),secondary_y=True)
    ...
    fig_comercio.add_trace(go.Bar(x=data.index,y=var_m['Comercio'],name='Var. Mensual',marker_color=var_m['Comercio'].apply(lambda x: 'green' if x >= 0 else 'red'),opacity=0.5,marker_line_color=black,marker_line_width=1),secondary_y=True)
    ...
    fig_inmob.add_trace(go.Bar(x=data.index,y=var_m['Inmobiliaria'],name='Var. Mensual',marker_color=var_m['Inmobiliaria'].apply(lambda x: 'green' if x >= 0 else 'red'),opacity=0.5,marker_line_color=black,marker_line_width=1),secondary_y=True)
...
def make_actividad_web():
    ...
    with st.container(border=True):
        ...
        if S.indicador_actividad=='EMAE':
            ...
            with st.container(border=True):st.metric(label=f"Último Dato ({actividad.index[-1]})",value=round(actividad.iloc[-1][_[S.emae_elegido]],2),delta=f"{round(var_men_act.iloc[-1][_[S.emae_elegido]],2)}%")
        else:
            with st.container(border=True):st.metric(label=f"Último Dato ({actividad.index[-1]})",value=round(actividad.iloc[-1][_[S.indicador_actividad]],2),delta=f"{round(var_men_act.iloc[-1][_[S.indicador_actividad]],2)}%")
    c1,c2=st.columns(2)
