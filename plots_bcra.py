...
def plot_varBM(escala,bcra:pd.DataFrame):
    ...
    bcra=bcra.rolling(roll).sum()#.dropna()
    fig = go.Figure()
...
def plot_reservas(reservas): # Está andando medio mal, no reacciona bien con el botón
    ...
    fig.add_trace(go.Scatter(x=reservas.loc["Jan-2002":].index, y=reservas["TCR"].rolling(12).mean().loc["Jan-2002":], name="Media anual-TCR", marker_color="darkgreen", line=dict(width=2.5,dash="dash")), secondary_y=False)
