...
def plot_reservas(reservas): # Está andando medio mal, no reacciona bien con el botón
    ...
    _,tcreq=sm.tsa.filters.hpfilter(reservas.dropna().TCR,129600)
    fig.add_trace(go.Scatter(x=reservas.index, y=reservas["TCR"], name="TCR", marker_color="#EF5A6F", line=dict(width=2)), secondary_y=True)
    fig.add_trace(go.Scatter(x=reservas.dropna().index, y=tcreq, name="TCR de equilibrio", marker_color="#D4BDAC", line=dict(width=2,dash="dash")), secondary_y=True)
