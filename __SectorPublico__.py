...
def plot_deuda(data,type_plot):
    if not isinstance(data,pd.DataFrame):st.error('Error Extrayendo los Datos de la Deuda Bruta')
        ...
        if type_plot=='Composición de la Deuda Bruta':
            ...
            fig.add_trace(go.Scatter(x=data.index,y=data['Total-Moneda Nacional'],name='Moneda Nacional',line=dict(width=2),marker_color='red',legendgroup='Moneda Nacional',fill="tozeroy"))
            fig.add_trace(go.Scatter(x=data.index,y=data['Total-Moneda Extranjera'],name='Moneda Extranjera',line=dict(width=2),marker_color='royalblue',legendgroup='Moneda Extranjera',fill="tozeroy"))
